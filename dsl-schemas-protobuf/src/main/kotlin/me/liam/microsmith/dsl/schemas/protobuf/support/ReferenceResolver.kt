...
fun getReferencePath(currentSegments: List<String>, target: String): List<String> = when {
    // absolute or unqualified
    !target.startsWith(".") -> if ('.' in target) {
        target.split('.')
    } else {
...
fun resolveReferences(schemas: Set<ProtobufSchema>): Set<ProtobufSchema> {
    ...
    messages.values.filterIsInstance<ProtobufMessageSchema>().forEach { schema ->
        schema.message.fields.filterIsInstance<ReferenceField>().forEach { it.reference.resolve() }

        schema.message.fields.filterIsInstance<MapField>().mapNotNull { it.type.value as? Reference }
            .forEach { it.resolve() }

        schema.message.oneofs.flatMap { it.fields }.mapNotNull { it.fieldType as? Reference }.forEach { it.resolve() }
    }
