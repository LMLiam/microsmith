...
class ProtobufDslIntegrationTests :
    StringSpec({
        "protobuf DSL builds message and enum schemas" {
            ...
            val person = schemas.schemas.filterIsInstance<ProtobufSchema>().first { it.schema is Message }
            val color = schemas.schemas.filterIsInstance<ProtobufSchema>().first { it.schema is Enum }

            person.name shouldBe "Person"
            person.schema.name shouldBe "Person"
            (person.schema as Message).fields.associate { it.name to it.index } shouldContainExactly
                mapOf(
                    "id" to 1,
                    "name" to 2,
                    "age" to 3,
            ...
            (color.schema as Enum).values.associate { it.name to it.index } shouldContainExactly
                mapOf(
                    "UNSPECIFIED" to 0,
                    "RED" to 1,
                    "GREEN" to 2
