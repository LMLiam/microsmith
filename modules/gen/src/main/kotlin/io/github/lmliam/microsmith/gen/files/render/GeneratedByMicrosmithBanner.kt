...
internal object GeneratedByMicrosmithBanner {
    ...
    private const val ORIGINS_HEADER = "Origins:"
    ...
    fun prepend(path: Path, contents: String, origins: Set<String> = emptySet()): String {
        val comment = resolveComment(path) ?: return contents
        if (contents.startsWith(commentText(comment, HEADER_TEXT))) {
            return contents
        }
        val bannerLines = buildList {
            add(commentText(comment, HEADER_TEXT))
            if (origins.isNotEmpty()) {
                add(commentText(comment, ORIGINS_HEADER))
                origins.toList().sorted().forEach { origin ->
                    add(commentText(comment, "- $origin"))
                }
            }
        }
        val banner = bannerLines.joinToString(separator = "\n")
